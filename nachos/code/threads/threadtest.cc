...
void MultipleSends(int param) {
  mailbox->Send(1);  
  mailbox->Send(2);
}

void MultipleReceives(int param) {
  int *message = new int;
  *message = 0;
...
void MailboxTest7() {
  ...
  Thread *T1 = new Thread("ONE");
  Thread *T2 = new Thread("TWO");
  T1->Fork(MultipleSends, 0);
  T2->Fork(MultipleReceives, 0);
}
...
// 3.3 should see prints (((1)))(((2)))(((3))) in that order
void ParentFork3_3(Thread *child){
	printf("(((1)))\n");
	child->Join();
	printf("(((3)))\n");
}
void ChildFork3_3(){printf("(((2)))\n");}
void Part3_3Test(){
	Thread *parent = new Thread("parent",0);
	Thread *child = new Thread("child",1);
	parent->Fork((VoidFunctionPtr)ParentFork3_2,(int)child);
