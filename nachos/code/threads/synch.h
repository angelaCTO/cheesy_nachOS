...
public:
	...
	// unique id of each thread, used for equating
...
//
//	Send() -- atomically waits until Receive is called on the
//	  same mailbox, then copies the message into the receive
//	  buffer. Once the copy is made, both can return.
//  Receive() -- waits until Send is called, at which point the copy
//    is made and both calls return.
class Mailbox {
public:
    Mailbox(char* name);
    ~Mailbox();
...
private:
    ...
    Lock *boxLock;      //Mailbox lock
    Lock *bufferLock;   //Message buffer lock
    int numItems;       //# items present in Mailbox
    int buffer;        //Message buffer
    int bufferSet;      //If buffer is set
    Condition* mailboxCV; //Mailbox condition
    Condition *bufferCV;  //Message buffer condition
    Semaphore *empty;   //If empty
    Semaphore *full;    //If full
};
#endif // SYNCH_H
